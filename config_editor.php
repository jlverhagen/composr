...
function do_access(string $given_password)
{
    $settings = [
        ...
        'use_persistent_database' => '<em>Database:</em> Whether to use persistent database connections (recommended only on dedicated servers experiencing high volumes of SQL traffic).',
        ...
        'use_persistent_cache' => '<em>Performance:</em> If persistent memory caching is to be used opposed to database cache (caches data in memory between requests using whatever appropriate PHP extensions are available). May be set to <kbd>0</kbd> to disable, <kbd>1</kbd> for auto-detection, or the name of a PHP file in <kbd>sources/hooks/systems/persistent_caching</kbd> to force a specific method (e.g. <kbd>apc</kbd>).',
        ...
        // TODO: disabled until the database API can be fixed
        /*
