...
/*
    This is specifically for downloading files from the downloads, not adding downloads. Use the content qualification if you want to track adding downloads.

    Supported parameters for this qualification:
    1) count  -- the number of unique files from the downloads system a member must download to meet this qualification (not specified: 10)
*/

/**
 * Hook class.
 */
class Hook_achievement_qualifications_download
{
    /**
    ...
    public function to_text(int $member_id, array $params, int $count_done, int $count_required) : ?object
    {
        ...
        return do_lang_tempcode('ACHIEVEMENT_DOWNLOAD_REQUIREMENT', protect_from_escaping($progress));
    }
}
