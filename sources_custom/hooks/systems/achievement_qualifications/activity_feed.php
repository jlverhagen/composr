...
/*
    ...
    1) count    -- The number of RAW_DUMP entries (status messages) a member must post for this qualification to be satisfied (not specified: 100)
    2) days     -- Only count status messages from the last specified number of days [persist is not supported if this is specified] (not specified: no filter)
*/

/**
 * Hook class.
 */
class Hook_achievement_qualifications_activity_feed
{
    /**
    ...
    public function run(int $member_id, array $params, ?int $last_time = null) : ?array
    {
        ...
        if ($days !== null) {
            ...
            $extra_where .= ' AND a_time>' . strval($last_time);
        }

        $count_done = $GLOBALS['SITE_DB']->query_select_value('activities', 'COUNT(*)', $where_map, $extra_where);
    ...
    public function to_text(int $member_id, array $params, int $count_done, int $count_required) : ?object
    {
        ...
        if ($days !== null) {
            $conditions->attach(do_lang_tempcode('ACHIEVEMENT_ACTIVITY_FEED_REQUIREMENT_DAYS', escape_html(integer_format($days))));
        }
        ...
        // Finalise
        return do_lang_tempcode('ACHIEVEMENT_ACTIVITY_FEED_REQUIREMENT', protect_from_escaping($conditions), protect_from_escaping($progress));
    }
}
