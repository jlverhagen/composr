...
/*
    ...
    1) count    -- The number of external tutorials a member must submit for this qualification to be satisfied (not specified: 1)
    2) views    -- Tutorials must get at least this many views before they are counted (not specified: 0)
    2) days     -- Only consider tutorials submitted within the past this many days [persist is not supported if this is specified] (not specified: no filter)
*/

/**
 * Hook class.
 */
class Hook_achievement_qualifications_tutorials
{
    /**
    ...
    public function run(int $member_id, array $params, ?int $last_time = null) : ?array
    {
        ...
        $views = isset($params['views']) ? intval($params['views']) : 0;
        ...
        if ($days !== null) {
            $extra_where .= ' AND t_add_date>=' . strval(time() - ($days * 24 * 60 * 60));
        } elseif ($last_time !== null) {
            $extra_where .= ' AND t_add_date>' . strval($last_time);
        }

        // Get results
        $count_done = $GLOBALS['SITE_DB']->query_select_value('tutorials_external', 'COUNT(*)', $where_map, $extra_where);
    ...
    public function to_text(int $member_id, array $params, int $count_done, int $count_required) : ?object
    {
        ...
        if ($views > 0) {
            $conditions->attach(do_lang_tempcode('ACHIEVEMENT_TUTORIALS_REQUIREMENT_VIEWS', escape_html(integer_format($views))));
        }
        if ($days !== null) {
            $conditions->attach(do_lang_tempcode('ACHIEVEMENT_TUTORIALS_REQUIREMENT_DAYS', escape_html(integer_format($days))));
        }
        ...
        // Finalise
        return do_lang_tempcode('ACHIEVEMENT_TUTORIALS_REQUIREMENT', protect_from_escaping($conditions), protect_from_escaping($progress));
    }
}
