...
class Module_cms_tutorials extends Source_standard_crud_module
{
    ...
    public function fill_in_edit_form(string $id)
    {
        ...
        if (!array_key_exists(0, $rows)) {
            warn_exit(do_lang_tempcode('MISSING_RESOURCE', 'tutorials_external', escape_html($id)));
        }
        $myrow = $rows[0];
    ...
    public function delete_actualisation(string $_id)
    {
        ...
        if ($title === null) {
            warn_exit(do_lang_tempcode('MISSING_RESOURCE', 'tutorials_external', escape_html($_id)));
        }
