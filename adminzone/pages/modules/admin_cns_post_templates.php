 ...
 * Module page class.
 */
class Module_admin_cns_post_templates extends Source_standard_crud_module
{
    ...
    public function get_entry_points(bool $check_perms = true, ?int $member_id = null, bool $support_crosslinks = true, bool $be_deferential = false) : ?array
    {
        ...
        return [
            'browse' => [do_lang_tempcode('menus:ITEMS_HERE', do_lang_tempcode('POST_TEMPLATES'), make_string_tempcode(escape_html(integer_format(intval($GLOBALS['FORUM_DB']->get_table_count_approx('f_post_templates')), 0)))), 'menu/adminzone/structure/forum/post_templates'],
    ...
    public function pre_run(bool $top_level = true, ?string $type = null) : ?object
    {
        ...
        if ($type == 'browse') {
            ...
            breadcrumb_set_parents([['_SEARCH:admin_cns_forums:browse', do_lang_tempcode('MANAGE_FORUMS')], ['_SELF:_SELF:browse', do_lang_tempcode('POST_TEMPLATES')]]);
        } elseif ($type == '_import') {
            breadcrumb_set_parents([['_SEARCH:admin_cns_forums:browse', do_lang_tempcode('MANAGE_FORUMS')], ['_SELF:_SELF:browse', do_lang_tempcode('POST_TEMPLATES')], ['_SELF:_SELF:import', do_lang_tempcode('IMPORT_STOCK_RESPONSES_PT')]]);
        } else {
            breadcrumb_set_parents([['_SEARCH:admin_cns_forums:browse', do_lang_tempcode('MANAGE_FORUMS')], ['_SELF:_SELF:browse', do_lang_tempcode('POST_TEMPLATES')]]);
        }
                    ...
                    for ($i = 0; $i < $zip_archive->numFiles; $i++) {
                        ...
                        if ((cms_strtolower_ascii(substr($filename, -4)) == '.txt') && (!should_ignore_file($filename))) {
                            ...
                            if ($data === false) { // Error reading
                                warn_exit(zip_error($tmp_name, ZipArchive::ER_READ));
                                break;
                            }
    ...
    public function create_selection_list_choose_table(array $url_map) : array
    {
        ...
        list($sql_sort, $sort_order, $sortable) = process_sorting_params('post_template', $current_ordering);
