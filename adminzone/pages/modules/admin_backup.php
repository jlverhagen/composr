...
class Module_admin_backup
{
    ...
    public function get_results() : object
    {
        ...
        if (!empty($entries)) {
            ...
            $files = do_template('COLUMNED_TABLE', ['_GUID' => '726070efa71843236e975d87d4a17dae', 'HEADER_ROW' => $header_row, 'ROWS' => $rows, 'NONRESPONSIVE' => false]);
        } else {
            $files = new Tempcode();
        }
    ...
    public function make_backup() : object
    {
        ...
        if ($b_type == 'full') {
            $file = 'Backup_full_' . date('Y-m-d', utctime_to_usertime()) . '__' . get_secure_random_string(); // The last bit is unfortunate, but we need to stop URL guessing
        } elseif ($b_type == 'incremental') {
            $file = 'Backup_incremental' . date('Y-m-d', utctime_to_usertime()) . '__' . get_secure_random_string(); // The last bit is unfortunate, but we need to stop URL guessing
        } elseif ($b_type == 'sql') {
