...
class Module_tickets
{
    ...
    protected function render_ticket_row(array $topic) : array
    {
        ...
        if (function_exists('get_homesite_support_timings_wrap')) { // FUDGE. Extra code may be added in for the homesite's ticket system
            $extra_details = get_homesite_support_timings_wrap($topic['closed'] == 0, $topic['id'], $ticket_type_name);
        } else {
    ...
    public function display_ticket() : object
    {
        ...
        $extra_details = new Tempcode();
        if (function_exists('get_homesite_support_timings_wrap')) { // FUDGE. Extra code may be added in for the homesite's ticket system
            if (!$starting_new_ticket && ($our_topic !== null)) {
                ...
                $extra_details = get_homesite_support_timings_wrap($our_topic['closed'] == 0, $our_topic['id'], $ticket_type_name, true);
            }
        }
    ...
    public function update_ticket() : object
    {
        ...
        if ($ticket_type_id === null) {
            if (addon_installed('stats')) {
                require_code('stats');
                if (is_integer($ticket_type_details['ticket_type_name'])) {
                    $ticket_type_name_default_lang = get_translated_text($ticket_type_details['ticket_type_name'], null, get_site_default_lang());
                } else {
                    $ticket_type_name_default_lang = $ticket_type_details['ticket_type_name'];
                }
                log_stats_event(do_lang('FORM', null, null, null, get_site_default_lang()) . '-' . $ticket_type_name_default_lang);
            }
        }
