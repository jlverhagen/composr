...
class Module_awards
{
    ...
    public function get_entry_points(bool $check_perms = true, ?int $member_id = null, bool $support_crosslinks = true, bool $be_deferential = false) : ?array
    {
        ...
        if ($check_perms) {
            if ($GLOBALS['SITE_DB']->get_table_count_approx('award_types') == 0) {
                return [];
            }
        }

        return [
            'browse' => ['AWARDS', 'menu/adminzone/setup/awards'],
            'overview' => ['AWARD_OVERVIEW', 'admin/view_archive'],
        ];
    }
    ...
    public function pre_run() : ?object
    {
        ...
        if ($type == 'award') {
            breadcrumb_set_parents([['_SELF:_SELF:browse', do_lang_tempcode('AWARDS')]]);
    ...
    public function choose_award() : object
    {
        ...
        foreach ($rows as $myrow) {
            ...
            $info = $ob->info();
            if ($info !== null) {
                $url = build_url(['page' => '_SELF', 'type' => 'award', 'id' => $myrow['id']], '_SELF');
        ...
        if (has_actual_page_access(get_member(), 'admin_awards')) {
            $add_url = build_url(['page' => 'admin_awards', 'type' => 'add'], get_module_zone('admin_awards'));
        }

        return do_template('INDEX_SCREEN_FANCIER_SCREEN', ['_GUID' => 'c8351f627333434d426db3b9ffe09d1c', 'ADD_URL' => $add_url, 'PRE' => '', 'POST' => '', 'TITLE' => $this->title, 'CONTENT' => $out]);
    }
    ...
    public function award_overview() : object
    {
        ...
        foreach ($award_types as $award_type_row) {
            ...
            $rows = $GLOBALS['SITE_DB']->query_select('award_archive', ['*'], ['a_type_id' => $award_type_row['id']], 'ORDER BY date_and_time DESC', 1);
            foreach ($rows as $myrow) {
                ...
                if ($award_content_row !== null) {
                    ...
                    $rendered = do_template('AWARDED_CONTENT', [
    ...
    public function award() : object
    {
        ...
        $rows = $GLOBALS['SITE_DB']->query_select('award_archive', ['*'], ['a_type_id' => $id], 'ORDER BY date_and_time DESC', $max, $start);
        $max_rows = $GLOBALS['SITE_DB']->query_select_value('award_archive', 'COUNT(*)', ['a_type_id' => $id]);
        $content = new Tempcode();
        foreach ($rows as $myrow) {
            require_code('content');
            $award_content_row = content_get_row($myrow['content_id'], $info);
            ...
            if ($award_content_row !== null) {
                ...
                $content->attach(do_template('AWARDED_CONTENT', [
