        ...
        switch ($next_token) {
            ...
            case '}':
                ...
                if ($past_level_mode === PARSE_DIRECTIVE) {
                    $tpl_funcs = [];
                    $eval = trim(tempcode_compiler_eval('return ' . $first_param . ';', $tpl_funcs, [], $cl));
                    if (!is_string($eval)) {
                        $eval = '';
                    }
                    if ($eval === 'START') { // START
                        ...
                        // Work out name
                        $tpl_funcs = [];
                        $eval = trim(tempcode_compiler_eval('return ' . implode('.', $directive_opener_params[1]) . ';', $tpl_funcs, [], $cl));
                        if (!is_string($eval)) {
                            $eval = '';
                        }
                        $directive_name = $eval;
