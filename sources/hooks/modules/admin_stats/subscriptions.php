 ...
 * Hook class.
 */
class Hook_admin_stats_subscriptions extends Source_hook_stats_provider
{
    /**
    ...
    public function info(bool $for_kpi = false) : ?array
    {
        ...
        return [
            'subscriptions' => [
                'label' => do_lang_tempcode('MODULE_TRANS_NAME_subscriptions'),
                'category' => 'conversions',
                'filters' => [
                    'subscriptions__day_range' => new Source_stats_filter_day_range('subscriptions__day_range', do_lang_tempcode('DATE_RANGE'), null, $for_kpi),
                ],
                'pivot' => new Source_stats_filter_date_pivot('subscriptions__pivot', $this->get_date_pivots(!$for_kpi)),
    ...
    public function preprocess_raw_data(int $start_time, int $end_time)
    {
        ...
        $query = 'SELECT * FROM ' . get_table_prefix() . 'ecom_subscriptions WHERE ';
        $query .= 's_time>=' . strval($start_time) . ' AND ';
        $query .= 's_time<=' . strval($end_time);
        $query .= ' ORDER BY s_time';
        do {
            ...
            foreach ($rows as $row) {
                ...
                foreach (array_keys($date_pivots) as $pivot) {
                    ...
                    if (!isset($this->data_buckets['subscriptions'][$pivot][$pivot_interval][$pivot_value])) {
                        $this->data_buckets['subscriptions'][$pivot][$pivot_interval][$pivot_value] = 0;
                    }
    ...
    public function generate_final_data(string $bucket, string $pivot, array $filters) : ?array
    {
        ...
        foreach ($_data as $_pivot => $__data) {
            foreach ($__data as $pivot_interval => $_) {
                foreach ($_ as $pivot_value => $total_subscriptions) {
                    ...
                    if ($total_subscriptions === null) {
                        continue;
                    }

                    $data[$pivot_value_nice] += $total_subscriptions;
                }
            }
        }
