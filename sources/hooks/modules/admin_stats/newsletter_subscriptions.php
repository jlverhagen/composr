...
class Hook_admin_stats_newsletter_subscriptions extends Source_hook_stats_provider
{
    ...
    public function preprocess_raw_data(int $start_time, int $end_time, array &$data_buckets)
    {
        ...
        $query .= 'join_time>=' . strval($start_time) . ' AND ';
        $query .= 'join_time<=' . strval($end_time);
        $query .= ' ORDER BY join_time';
        do {
            ...
            foreach ($rows as $row) {
                ...
                foreach (array_keys($date_pivots) as $pivot) {
                    ...
                    if (!isset($data_buckets['newsletter_subscriptions'][$pivot][$pivot_interval][$pivot_value])) {
                        $data_buckets['newsletter_subscriptions'][$pivot][$pivot_interval][$pivot_value] = 0;
                    }
                    $data_buckets['newsletter_subscriptions'][$pivot][$pivot_interval][$pivot_value]++;
                }
    ...
    public function generate_final_data(string $bucket, string $pivot, array $filters) : ?array
    {
        ...
        foreach ($_data as $_pivot => $__data) {
            foreach ($__data as $pivot_interval => $_) {
                foreach ($_ as $pivot_value => $num_subscriptions) {
                    ...
                    if ($num_subscriptions === null) {
                        continue;
                    }

                    $data[$pivot_value_nice] += $num_subscriptions;
                }
            }
        }
