...
class Hook_import_html_site
{
    ...
    public function import_pages(object $db, string $table_prefix, string $file_base)
    {
        ...
        foreach ($files as $i => $file) {
            if ((cms_strtolower_ascii(substr($file, -4)) == '.htm') || (cms_strtolower_ascii(substr($file, -5)) == '.html') || (cms_strtolower_ascii(substr($file, -4)) == '.php')) {
                $content_files[] = $file;
                unset($files[$i]);
            }
        }
        ...
        if (count($compare_file_contents) > 1) {
            ...
            // Strip bits
            $regexp_1 = '#^[\|\-' . ((get_charset() == 'utf-8') ? "\u{2013}\u{2014}" : '') . ',]#';
            $regexp_2 = '#[\|\-' . ((get_charset() == 'utf-8') ? "\u{2013}\u{2014}" : '') . '] )?' . preg_quote($site_name) . ',]$#';
        ...
        foreach ($content_files as $content_file) {
            ...
            if (substr($content_file, -4) == '.php') {
                ...
                if (preg_match('#<title>(.*)</title>#', $filtered, $matches) != 0) {
                    $regexp = '#( [\|\-' . ((get_charset() == 'utf-8') ? "\u{2013}\u{2014}" : '') . '] )?' . preg_quote($site_name) . '( [\|\-' . ((get_charset() == 'utf-8') ? (hex2bin('e28093') . hex2bin('e28094')) : '') . '] )?' . preg_quote($site_name) . '] )?#';
