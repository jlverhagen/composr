...
class Hook_import_mybb
{
    ...
    public function import_cns_groups(object $db, string $table_prefix, string $file_base)
    {
        ...
        $rows = $db->query_select('usergroups', ['*'], [], 'ORDER BY gid');
        foreach ($rows as $row) {
        ...
        do {
            ...
            foreach ($rows as $row) {
                ...
                $id_new = cns_make_member(
                    ...
                    1, // auto_mark_read
                    null, // sound_enabled
                    $row['receivepms'], // allow_emails
                    $row['receivepms'], // allow_emails_from_staff
                    0, // highlighted_name
                    '*', // pt_allow
                    '', // pt_rules_text
                    $validated, // validated
    ...
    public function import_cns_forums(object $db, string $table_prefix, string $old_base_dir)
    {
        ...
        $rows = $db->query_select('forums', ['*'], ['type' => 'f']);
        foreach ($rows as $row) {
            ...
            $access_mapping = [];
            if ($row['status'] == 1) {
                $permissions = $db->query_select('forumpermissions', ['*'], ['fid' => $row['fid']]);
    ...
    public function import_cns_topics(object $db, string $table_prefix, string $file_base)
    {
        ...
        $rows = [];
        do {
            $rows = $db->query_select('threads', ['*'], ['visible' => 1], 'ORDER BY tid', 200, $row_start);
            foreach ($rows as $row) {
                ...
                $id_new = cns_make_topic($forum_id, $row['subject'], '', 1, ($row['visible'] == 1) ? 0 : 1, 0, 0, null, null, false, $row['views']);
    ...
    public function import_cns_posts(object $db, string $table_prefix, string $file_base)
    {
        ...
        $rows = [];
        do {
            $rows = $db->query_select('posts p', ['*'], [], 'ORDER BY p.pid', 200, $row_start);
            foreach ($rows as $row) {
                ...
                $title = '';
                $topics = $db->query_select('threads', ['subject'], ['tid' => $row['tid']]);
    ...
    public function fix_links(string $post, object $db, string $table_prefix) : string
    {
        ...
        $post = preg_replace_callback('#' . preg_quote($OLD_BASE_URL) . '/(showthread\.php\?tid=)(\d*)#', [$this, '_fix_links_callback_topic'], $post);
        $post = preg_replace_callback('#' . preg_quote($OLD_BASE_URL) . '/(forumdisplay\.php\?fid=)(\d*)#', [$this, '_fix_links_callback_forum'], $post);
        $post = preg_replace_callback('#' . preg_quote($OLD_BASE_URL) . '/(member\.php\?action=profile\&uid=)(\d*)#', [$this, '_fix_links_callback_member'], $post);
    ...
    public function import_cns_post_files(object $db, string $table_prefix, string $file_base)
    {
        ...
        $rows = [];
        do {
            $rows = $db->query_select('attachments', ['*'], [], 'ORDER BY aid', 200, $row_start);
            foreach ($rows as $row) {
                ...
                $attachment_map = ['a_member_id' => $member_id, 'a_file_size' => $row['filesize'], 'a_url' => $url, 'a_thumb_url' => $thumb_url, 'a_original_filename' => $row['filename'], 'a_num_downloads' => $row['downloads'], 'a_last_downloaded_time' => null, 'a_add_time' => $row['dateuploaded'], 'a_description' => ''];
    ...
    public function import_cns_polls_and_votes(object $db, string $table_prefix, string $file_base)
    {
        ...
        foreach ($rows as $row) {
            ...
            $rows2 = $db->query_select('pollvotes', ['*'], ['pid' => $row['pid']]);
    ...
    public function import_notifications(object $db, string $table_prefix, string $file_base)
    {
        ...
        $rows = [];
        do {
            $rows = $db->query_select('threadsubscriptions', ['*'], [], '', 200, $row_start);
            foreach ($rows as $row) {
    ...
    public function import_custom_comcode(object $db, string $table_prefix, string $file_base)
    {
        ...
        foreach ($rows as $row) {
            ...
            $test = $GLOBALS['SITE_DB']->query_select_value_if_there('custom_comcode', 'tag_tag', ['tag_tag' => $custom_tag]);
    ...
    public function import_cns_custom_profile_fields(object $db, string $table_prefix, string $file_base)
    {
        ...
        foreach ($rows as $row) {
            ...
            $id_new = $GLOBALS['FORUM_DB']->query_select_value_if_there('f_custom_fields', 'id', [$GLOBALS['FORUM_DB']->translate_field_ref('cf_name') => $row['name']]);
            if ($id_new === null) {
