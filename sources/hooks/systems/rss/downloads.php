...
class Hook_rss_downloads
{
    ...
    public function run(string $_filters, int $cutoff, string $prefix, string $date_string, int $max) : ?array
    {
        ...
        $query = 'SELECT *,d.id AS d_id FROM ' . $GLOBALS['SITE_DB']->get_table_prefix() . 'download_downloads d WHERE add_date>' . strval($cutoff) . (((addon_installed('validation')) && (!has_privilege(get_member(), 'see_not_validated'))) ? ' AND validated=1 ' : '') . ' AND ' . $filters . ' ORDER BY add_date DESC';
