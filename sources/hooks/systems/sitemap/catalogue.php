 ...
 * Hook class.
 */
class Hook_sitemap_catalogue extends Source_hook_sitemap_content
{
    ...
    public function get_virtual_nodes(string $page_link, $callback = null, ?array $valid_node_types = null, ?int $child_cutoff = null, ?int $max_recurse_depth = null, int $recurse_level = 0, int $options = 0, string $zone = '_SEARCH', int $meta_gather = 0, bool $return_anyway = false) : ?array
    {
        ...
        $start = 0;
        do {
            $rows = $GLOBALS['SITE_DB']->query_select('catalogues', $select, $map, '', SITEMAP_MAX_ROWS_PER_LOOP, $start);
            foreach ($rows as $row) {
    ...
    public function get_node(string $page_link, $callback = null, ?array $valid_node_types = null, ?int $child_cutoff = null, ?int $max_recurse_depth = null, int $recurse_level = 0, int $options = 0, string $zone = '_SEARCH', int $meta_gather = 0, ?array $row = null, bool $return_anyway = false) : ?array
    {
        ...
        if (strpos($page_link, ':index:') !== false) {
            ...
            if (($meta_gather & SITEMAP_GATHER_IMAGE) != 0) {
                $test_icon = 'menu/rich_content/catalogues/' . $content_id;
                $test_image = find_theme_image('icons/' . $test_icon, true);
                if ($test_image == '') {
                    $test_icon = 'menu/rich_content/catalogues/catalogues';
                    $test_image = find_theme_image('icons/' . $test_icon, true);
                }
                if ($test_image != '') {
                    $struct['extra_meta']['image'] = $test_image;
            ...
            if (($meta_gather & SITEMAP_GATHER_IMAGE) != 0) {
                $test_icon = 'menu/rich_content/atoz';
                $test_image = find_theme_image('icons/' . $test_icon, true);
                if ($test_image != '') {
                    $struct['extra_meta']['image'] = $test_image;
                    $struct['extra_meta']['icon'] = $test_icon;
