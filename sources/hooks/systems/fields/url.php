...
class Hook_fields_url
{
    ...
    public function get_field_inputter(string $_cf_name, string $_cf_description, array $field, ?string $actual_value, bool $new) : ?object
    {
        ...
        return form_input_url($_cf_name, $_cf_description, $input_name, $actual_value, $field['cf_required'] == 1, null, $autocomplete);
    }
    ...
    public function inputted_to_field_value(bool $editing, array $field, ?string $upload_dir = 'uploads/catalogues', ?array $old_value = null) : ?string
    {
        ...
        if ($value != STRING_MAGIC_NULL) {
            ...
            if (addon_installed('health_check') && ($value != '')) {
                $message = '';
                require_code('urls2');
                require_lang('health_check');
                $ok = check_url_exists($value, 60 * 60 * 24 * 1, true, 3, $message);
                if ($message === null) {
                    $message = 'UNKNOWN';
                }
                if (!$ok) {
                    $message_text = do_lang('BROKEN_LINK_PROBLEM__' . str_replace('-', '_', $message), $value, $message, null, null, false); // Support custom messages for specific status codes
                    if ($message_text === null) {
                        $message_text = do_lang('BROKEN_LINK_PROBLEM', $value, $message);
                    }
