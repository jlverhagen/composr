 ...
 * Hook class.
 */
class Hook_spam_heuristics_country
{
    /**
    ...
    {
        $score = intval(get_option('spam_heuristic_confidence_country'));
        if ($score == 0) {
            return 0;
        }

        $countries = get_option('spam_heuristic_country');
        if ($countries != '') {
            require_code('locations');
            $user_country = get_country();

            foreach (explode(',', $countries) as $country) {
                if ($user_country == trim($country)) {
                    return $score;
                }
            }
        }
