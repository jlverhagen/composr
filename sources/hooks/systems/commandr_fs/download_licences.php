 ...
 * Hook class.
 */
class Hook_commandr_fs_download_licences extends Source_resource_fs_base
{
    ...
    {
        $_ret = $GLOBALS['SITE_DB']->query_select('download_licences', ['id'], ['l_title' => $label], 'ORDER BY id');
        $ret = [];
        foreach ($_ret as $r) {
            $ret[] = strval($r['id']);
        }
    ...
    public function file_load(string $filename, string $path)
    {
        ...
        $rows = $GLOBALS['SITE_DB']->query_select('download_licences', ['*'], ['id' => intval($resource_id)], '', 1);
