 ...
 * @package core_database_drivers
 */
class Source_database_static_mysqli extends Source_database_super_mysql
{
    protected $cache_db = [];
    ...
    public function query(string $query, $connection, ?int $max = null, int $start = 0, bool $fail_ok = false, bool $get_insert_id = false, bool $save_as_volatile = false)
    {
        ...
        if ($this->version === null) {
            $this->version = mysqli_get_server_version($db_link);
        }
        if ($this->version >= 80000) {
        ...
        if (($results === false) && ((!$fail_ok) || (strpos(mysqli_error($db_link), 'is marked as crashed and should be repaired') !== false))) {
            ...
            if (in_array($err, ['MySQL server has gone away', 'Lost connection to MySQL server during query']) && (!$this->reconnected_once)) {
