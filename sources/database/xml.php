...
function init__database__xml()
{
    ...
    $SYMBOL_DELIMITER = array_flip(["\t", ' ', "\n", '+', '-', '*', '/', '>', '<', '=', "'", '"', "\\'", '(', ')', ',']);
    foreach ($DELIMITERS as $d) {
        if (!isset($DELIMITERS_ALPHA[$d[0]])) {
            $DELIMITERS_ALPHA[$d[0]] = [];
        }
    ...
    $INT_TYPES = ['AUTO', 'AUTO_LINK', 'INTEGER', 'UINTEGER', 'SHORT_INTEGER', 'BINARY', 'MEMBER', 'GROUP', 'TIME', 'integer'];
...
class Source_database_static_xml extends Source_database_driver
{
    ...
    public function get_type_remap(bool $for_alter = false) : array
    {
        $type_remap = [
            ...
            'LONG_TRANS__COMCODE' => 'LONG_TRANS__COMCODE',
            'SHORT_TRANS__COMCODE' => 'SHORT_TRANS__COMCODE',
    ...
    protected function _read_schema(array $db, string $table_name, bool $fail_ok = false) : ?array
    {
        ...
        if ($table_name == $table_prefix . 'db_meta') {
            ...
            if ($fields === null) {
                return []; // Can happen during installation
            }
        }
    ...
    protected function _type_check(array $schema, array $record, string $query)
    {
        ...
        foreach ($record as $key => $val) {
            if (!isset($schema[$key])) {
                $this->failed_query_exit('Unrecognised key, ' . $key);
            }
        ...
        if (($schema !== null) && ($where_expr !== null)) { // Try for an efficient filename-based lookup
            ...
            if (($key_lookup) && ($key_buildup != '')) {
                ...
                if (($file_exists_xml) || ($file_exists_xml_volatile)) {
                    ...
                    $test = $this->_read_record($db[0] . '/' . $table_name . '/' . $key_buildup . $suffix, $schema, [], $include_unused_fields, $fail_ok);
                    if ($test === null) {
                        return [];
                    }
                    $records[$the_key] = $test;
                    if ($table_name == $this->table_prefix . 'translate') {
                        $sup_file = $db[0] . '/' . $table_name . '/sup/' . $key_buildup . '.xml-volatile';
                        if (file_exists($sup_file)) {
                            $sup_record = $this->_read_record($sup_file, $schema, [], $include_unused_fields, $fail_ok);
        ...
        foreach ($files as $file) {
            ...
            if ($read !== null) {
                ...
                if ($table_name == $this->table_prefix . 'translate') {
                    ...
                    if (file_exists($sup_file)) {
                        $test = $this->_read_record($sup_file, $schema, [], $include_unused_fields, $fail_ok);
                        if ($test !== null) {
                            $records[$the_key] += $test;
                        }
                    }
                ...
                if (($max !== null) && (count($records) >= $max) && (($where_expr === null) || ($where_expr == ['LITERAL', true]))) {
                    break;
                }
            }
        }
    ...
    protected function _turn_where_expr_to_map(array $where_expr, string $table_as, ?array $schema = null, bool $not_full_accuracy = false) : array
    {
        ...
        if ($where_expr[0] == 'OR') {
            ...
            if ((count($alpha) == 1) && (count($beta) == 1) && ($_alpha == $_beta)) {
                $alpha[$_alpha[0]] = array_merge(is_array($alpha[$_alpha[0]]) ? $alpha[$_alpha[0]] : [$alpha[$_alpha[0]]], is_array($beta[$_beta[0]]) ? $beta[$_beta[0]] : [$beta[$_beta[0]]]);
                return $alpha;
            }
        }
        if (($where_expr[0] == '=') && ($where_expr[1][0] == 'LITERAL') && ($where_expr[2][0] == 'FIELD')) {
            $where_expr = [$where_expr[0], $where_expr[2], $where_expr[1]];
        }
        if (($where_expr[0] == 'LIKE') && ($where_expr[1][0] == 'FIELD') && ($where_expr[2][0] == 'LITERAL') && ($not_full_accuracy)) {
            ...
            if ($table_as != '') {
                ...
                if (strpos($key, '.') !== false) {
                    return []; // Not for our table
                }
            }

            if (($schema !== null) && (!array_key_exists($key, $schema))) {
                return []; // Not in our table (join involved. must be in other join)
            }
    ...
    protected function _read_record(string $path, ?array $schema = null, array $must_contain_strings = [], bool $include_unused_fields = false, bool $fail_ok = false) : ?array
    {
        ...
        $file_contents = @cms_file_get_contents_safe($path, FILE_READ_LOCK | FILE_READ_BOM);
        ...
        */
        ...
        if ($schema === null) {
            ...
            foreach ($_record as $key => $val) {
                ...
                } elseif (in_array($schema_type, ['REAL'])) {
                    if ((($val === null) || ($val === '')) && (substr($type, 0, 1) == '?')) {
                        $new_val = null;
                    } else {
    ...
    protected function _write_record(array $db, string $table_name, string $guid, array $record, bool $fail_ok = false, bool $save_as_volatile = false)
    {
        ...
        if ($table_name == $this->table_prefix . 'translate') { // Special code to store volatile text_parsed attribute externally
            $record_copy = $record;
    ...
    protected function _do_query_insert__parse(array $tokens, string $query, array $db, bool $fail_ok) : ?array
    {
        ...
        do {
            ...
            do {
                ...
                $result = $this->_execute_expression($expr, [], $query, $db, $fail_ok);
    ...
    protected function _parsing_read_expression(int &$at, array $tokens, string $query, array $db, bool $look_for_connectives = true, bool $look_for_any_connectives = true, bool $fail_ok = false) : ?array
    {
        ...
        switch ($token) {
            ...
            case 'COUNT':
                ...
                if ($_token == 'DISTINCT') {
                    $is_distinct = true;
                    $being_counted = $this->_parse_select_items($at, $tokens, $query, $db, $fail_ok);
                } else {
                    $is_distinct = false;
                    $being_counted = $token;
                }
                $expr = [$token, $being_counted, $is_distinct];
                if (!$this->_parsing_expects($at, $tokens, ')', $query)) {
                    return null;
                }
                break;
            ...
            case '(':
                ...
                if ($next_token == 'SELECT') { // subquery
                    ...
                    $expr = ['SUBQUERY_VALUE', $subquery];
                } else {
                    $expr = ['PARENTHESISED', $this->_parsing_read_expression($at, $tokens, $query, $db, true, true, $fail_ok)];
                }
                if (!$this->_parsing_expects($at, $tokens, ')', $query)) {
                    return null;
                }
            ...
            case 'NOT':
                $expr = ['NOT', $this->_parsing_read_expression($at, $tokens, $query, $db, false, true, $fail_ok)];
                break;
                ...
                if (!in_array($token, $all_keywords)) { // Must be a field reference then
                    if (is_numeric($token)) {
                        if (strpos($token, '.') !== false) {
                            $expr = ['LITERAL', floatval($token)];
                        } else {
                            $expr = ['LITERAL', intval($token)];
                        }
                    } else {
                        if (substr($token, -1) == '.') {
                            $token .= $this->_parsing_read($at, $tokens, $query);
        ...
        if ($look_for_any_connectives) {
            ...
            switch ($token) {
                ...
                case '<>':
                case 'LIKE':
                    $expr = [$token, $expr, $this->_parsing_read_expression($at, $tokens, $query, $db, false, true, $fail_ok)];
                    break;

                case '*':
                    $expr = ['MULTI', $expr, $this->_parsing_read_expression($at, $tokens, $query, $db, false, true, $fail_ok)];
                    break;
    ...
    protected function _execute_expression(array $expr, array $bindings, string $query, array $db, bool $fail_ok, ?array $full_set = null, bool &$cuts_to_single_row = false)
    {
        switch ($expr[0]) {
            ...
            case 'FIELD':
                if (!array_key_exists($expr[1], $bindings)) {
                    $this->_bad_query($query, false, 'Missing referenced field, ' . $expr[1]);
                }
            ...
            case 'LIKE':
                ...
                return simulated_wildcard_match($value, $expr_eval, true, true);

            case 'EXISTS':
                list($exists_select, $exists_as, $exists_joins, $exists_where_expr, $exists_group_by, $exists_having, $exists_orders, $exists_unions, $exists_start, $exists_max, $exists_is_distinct) = $expr[1];
                $exists_results = $this->_execute_query_select($exists_select, $exists_as, $exists_joins, $exists_where_expr, $exists_group_by, $exists_having, $exists_orders, $exists_unions, $query, $db, $exists_max, $exists_start, $exists_is_distinct, $bindings, $fail_ok);
            ...
            case 'SUBQUERY_VALUE':
                list($subquery_select, $subquery_as, $subquery_joins, $subquery_where_expr, $subquery_group_by, $subquery_having, $subquery_orders, $subquery_unions, $subquery_start, $subquery_max, $subquery_is_distinct) = $expr[1];
                $subquery = $this->_execute_query_select($subquery_select, $subquery_as, $subquery_joins, $subquery_where_expr, $subquery_group_by, $subquery_having, $subquery_orders, $subquery_unions, $query, $db, $subquery_max, $subquery_start, $subquery_is_distinct, $bindings, $fail_ok);
                if ($subquery === null) {
                    return null;
                }
            ...
            case 'IN_SUBQUERY':
                ...
                list($subquery_select, $subquery_as, $subquery_joins, $subquery_where_expr, $subquery_group_by, $subquery_having, $subquery_orders, $subquery_unions, $subquery_start, $subquery_max, $subquery_is_distinct) = $expr[2];
                $results = $this->_execute_query_select($subquery_select, $subquery_as, $subquery_joins, $subquery_where_expr, $subquery_group_by, $subquery_having, $subquery_orders, $subquery_unions, $query, $db, $subquery_max, $subquery_start, $subquery_is_distinct, $bindings, $fail_ok);
    ...
    protected function _do_query_update(array $tokens, string $query, array $db, ?int $max, int $start, bool $fail_ok)
    {
        ...
        $records = $this->_read_all_records($db, $table_name, '', $schema, $where_expr, [], $fail_ok, $query, false, $max);
        if ($records === null) {
            return null;
        }
        $i = 0;
    ...
    protected function _do_query_select(array $tokens, string $query, array $db, ?int $max, int $start, bool $fail_ok, int &$at, bool $do_end_check = true)
    {
        ...
        list($select, $as, $joins, $where_expr, $group_by, $having, $orders, $unions, $start, $max, $is_distinct) = $test;
        return $this->_execute_query_select($select, $as, $joins, $where_expr, $group_by, $having, $orders, $unions, $query, $db, $max, $start, $is_distinct, [], $fail_ok);
    }
    ...
    protected function _parse_query_select(array $tokens, string $query, array $db, ?int $max, int $start, bool $fail_ok, int &$at, bool $do_end_check = true) : ?array
    {
        ...
        $select = $this->_parse_select_items($at, $tokens, $query, $db, $fail_ok);
        ...
        return [$select, $as, $joins, $where_expr, $group_by, $having, $orders, $unions, $start, $max, $is_distinct];
    }

    /**
     * Parse the selected items from a SELECT query's selection clause.
     *
     * @param  integer $at Our offset counter
     * @param  array $tokens Tokens
     * @param  string $query Query that was executed
    ...
    protected function _parse_select_items(int &$at, array $tokens, string $query, array $db, bool $fail_ok) : array
    {
        ...
        do {
            ...
            if ($token == '*') {
                ...
                $select[] = ['*', substr($token, 0, strlen($token) - 2)];
            } else {
                $at--;
                ...
                if ($as_token === null) { // reached end of query
                    ...
                    if ($as_token == 'AS') {
                        ...
                        $select[] = ['AS', $expression, $as];
                    } elseif (($as_token == '*') && (substr($token, -1) == '.')) {
                        $select[] = ['*', substr($token, 0, strlen($token) - 1)];
                    } else {
                        $at--;
                        $select[] = $expression;
                    }
    ...
    protected function _execute_query_select(array $select, ?string $as, array $joins, array $where_expr, ?array $group_by, ?array $having, ?array $orders, array $unions, string $query, array $db, ?int $max, int $start, bool $is_distinct, array $bindings, bool $fail_ok, ?array &$schema = null)
    {
            ...
            foreach ($joins as $join) {
                if ($join[0] == 'SIMPLE') {
                    ...
                    if (is_array($join[1])) {
                        ...
                        list($join_select, $join_as, $join_joins, $join_where_expr, $join_group_by, $join_having, $join_orders, $join_unions, $join_start, $join_max, $join_is_distinct) = $join[1];
                        $records = $this->_execute_query_select($join_select, $join_as, $join_joins, $join_where_expr, $join_group_by, $join_having, $join_orders, $join_unions, $query, $db, $join_max, $join_start, $join_is_distinct, $bindings, $fail_ok);
                    } else {
        ...
        // Find what fields are being selected (we need this for later checks)
        $selected_fields = [];
        $selected_fields_aliases = [];
        $has_wildcard_selected_fields = false;
        foreach ($select as $i => $want) {
            ...
            switch ($want[0]) { // NB: COUNT, SUM, etc, already have their values rolled out into $record and we do not need to consider it here
                ...
                case 'AS':
                    $selected_fields_aliases[] = preg_replace('#^.*\.#', '', $want[2]);
                    ...
                    switch ($want[1][0]) {
                        ...
                        case 'AVG':
                            break 2;
                        default:
                            if ((isset($want[1][1])) && (is_string($want[1][1]))) {
                                $selected_fields[] = preg_replace('#^.*\.#', '', $want[1][1]);
                            }
                            break 2;
                    }
                    // no break
                ...
                default:
                    if ((isset($want[1])) && (is_string($want[1]))) {
                        $selected_fields[] = preg_replace('#^.*\.#', '', $want[1]);
                    }
                    break;
            }
        }
        ...
        if ($group_by !== null) {
            ...
            // Test that we aren't selecting records outside the scope of the GROUP BY
            foreach ($selected_fields as $selected_field) {
                if (!in_array($selected_field, preg_replace('#^.*\.#', '', $group_by))) {
                    return $this->_bad_query($query, $fail_ok, 'Selecting ' . $selected_field . ', which is not covered in GROUP BY clause');
                }
            }
            ...
            if ($is_distinct) {
                ...
                foreach ($records as $set_item) {
                    ...
                    $index[serialize($distinct_compound_val)] = $set_item;
                }
            ...
            foreach ($records as $i => $record) {
                $records[$i] = $this->_function_set_scoping($records, $select, $record, $query, $db, $fail_ok); // (*MARKER*)
                if ($single_result) {
                    $records = [$i => $records[$i]];
                    break;
                }
        ...
        foreach ($records as $record) {
            ...
            foreach ($select as $i => $want) {
                ...
                switch ($want[0]) { // NB: COUNT, SUM, etc, already have their values rolled out into $record and we do not need to consider it here
                    ...
                    case '*':
                        $_record += $this->_handle_multi_select($want, $record);
                        break;
        ...
        if ((empty($results)) && ($group_by === null)) {
            $rep = $this->_function_set_scoping([], $select, [], $query, $db, $fail_ok);
            if (!empty($rep)) {
                foreach ($select as $i => $want) {
                    $as = null;
                    if ($want[0] == 'AS') {
        ...
        foreach ($unions as $union) {
            ...
            list($union_select, $union_as, $union_joins, $union_where_expr, $union_group_by, $union_having, $union_orders, $union_unions, $union_start, $union_max, $union_is_distinct) = $test;

            $results_b = $this->_execute_query_select($union_select, $union_as, $union_joins, $union_where_expr, $union_group_by, $union_group_by, $union_orders, $union_unions, $query, $db, $union_max, $union_start, $union_is_distinct, $bindings, $fail_ok);
            if ($results_b === null) {
                return null;
            }
    ...
    /**
     * Handle row field select for some kind of '*' SELECT clause.
     *
     * @param  array $want Our selector
            ...
            switch ($s_term[0]) {
                ...
                case 'COUNT':
                    if ($s_term[2]) {
                        ...
                        foreach ($set as $set_item) {
                            $val = [];
                            foreach ($s_term[1] as $j => $want) {
                                switch ($want[0]) { // NB: COUNT, SUM, etc, already have their values rolled out into $record and we do not need to consider it here
                                    case '*':
                                        $val += $this->_handle_multi_select($want, $set_item);
                                        break;
                                    ...
                                    default:
                                        $val[] = $this->_execute_expression($want, $set_item, $query, $db, $fail_ok, $set);
                                        break;
                                }
                            }
    ...
    protected function _read_join(int &$at, array $tokens, string $query, array $db, bool $fail_ok, int &$closing_parentheses_needed) : ?array
    {
        ...
        if ($join_table == '(') {
            ...
            if ($join_table == 'SELECT') {
                $at--;

                $join_table = $this->_parse_query_select($tokens, $query, $db, 1, 0, $fail_ok, $at, false);
        ...
        switch ($token) {
            case ',':
                $join = ['JOIN', $join_table, $joined_as, $on_expr];
                break;
            case 'JOIN':
                $join = ['JOIN', $join_table, $joined_as, $on_expr];
                break;
            case 'LEFT':
                $join = ['LEFT_JOIN', $join_table, $joined_as, $on_expr];
                break;
            case 'RIGHT':
                $join = ['RIGHT_JOIN', $join_table, $joined_as, $on_expr];
                break;
            case 'INNER':
                $join = ['INNER_JOIN', $join_table, $joined_as, $on_expr];
                break;
        }
    ...
    protected function _setify_join_condition_for_optimisation(array $join_condition, array $schema, array $records, string $joined_as) : array
    {
        if ($join_condition[0] == 'AND') {
            ...
            $join_condition = ['AND', $join_condition_a, $join_condition_b];
        } else {
            if ($join_condition[0] == '=') {
                foreach ([1, 2] as $i) {
                    if (($join_condition[$i][0] == 'FIELD') && ($join_condition[3 - $i][0] == 'FIELD')) { // If this and other-side expression are both FIELD's
                        ...
                        if (array_key_exists($var, $schema)) { // If this side is in the schema
                            $join_condition[$i][1] = []; // We'll make it a list instead of a field reference
    ...
    protected function _execute_join(array $db, string $joined_as_prior, array $join, string $query, array $records, array $schema, array $where_expr, array $bindings, bool $fail_ok = false) : ?array
    {
        ...
        if (is_array($join[1])) {
            // Actually a SELECT (subquery)
            list($select, $table, $joins, $where_expr, $group_by, $having, $orders, $unions, $start, $max, $is_distinct) = $join[1];
            $schema_b = [];
            $records_b = $this->_execute_query_select($select, $joined_as, $joins, $where_expr, $group_by, $having, $orders, $unions, $query, $db, $max, $start, $is_distinct, [], $fail_ok, $schema_b);
            $schema_b_plus = $schema_b;
            foreach ($schema_b as $k => $v) {
                $schema_b_plus[$joined_as . '.' . $k] = $v; // Needed so all scoped variables can be put in place as NULL's in a right variable
            }
            $schema += $schema_b_plus;
        } else {
            ...
            if ($where_expr == ['LITERAL', true]) {
                $where_expr_combined = $join_condition;
            } else {
                $where_expr_combined = ['AND', $where_expr, $join_condition];
            }
