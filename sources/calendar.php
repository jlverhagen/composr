...
function render_event_box(array $row, string $zone = '_SEARCH', bool $give_context = true, string $guid = '', ?object $text_summary = null) : object
{
    ...
    $url = build_url(['page' => 'calendar', 'type' => 'view', 'id' => $row['id']], $zone);
    ...
    return do_template('CALENDAR_EVENT_BOX', [
    ...
    for ($i = ($week == 52) ? 350/*conditional to stop it finding week as previous year overlap week of same number*/ : -7; $i < $days_in_year + 7; $i++) {
        ...
        if ((($ssw) && ($w == 0)) || ((!$ssw) && ($w == 1))) {
            ...
            if ($test == $basis) {
                ...
                return [intval($exploded[0]), intval($exploded[1]), intval($exploded[2])];
            }
        }
    }
...
function find_periods_recurrence(string $timezone, int $do_timezone_conv, int $start_year, int $start_month, int $start_day, string $start_monthly_spec_type, ?int $start_hour, ?int $start_minute, ?int $end_year, ?int $end_month, ?int $end_day, string $end_monthly_spec_type, ?int $end_hour, ?int $end_minute, string $recurrence, ?int $recurrences, ?int $period_start = null, ?int $period_end = null, ?int $max = null) : array
{
    ...
    if (($start_hour === null) && (($end_year === null) || ($end_month === null) || ($end_day === null))) { // All day event with no end date, should be same as start date.
        ...
        */
        ...
        if ($mask_covers) {
            if ($starts_within || $ends_within || $spans) {
                $times[] = [max($period_start, $a), min($period_end, $b), $a, $b, $_a, $_b];
            }
            $happened_count++;
        }
        $i++;
        ...
        if ($start_monthly_spec_type == 'day_of_month') {
            ...
            list($start_minute, $start_hour, $start_month, $start_day, $start_year) = normalise_time_array([$start_minute, $start_hour, $start_month, $start_day, $start_year], $timezone);
        } else {
            $_start_hour = ($start_hour === null) ? find_timezone_start_hour_in_utc($timezone, $start_year, $start_month, $start_day, $start_monthly_spec_type) : $start_hour;
            $_start_minute = ($start_minute === null) ? find_timezone_start_minute_in_utc($timezone, $start_year, $start_month, $start_day, $start_monthly_spec_type) : $start_minute;
            $start_day_of_month = find_concrete_day_of_month($start_year, $start_month, $start_day, $start_monthly_spec_type, $_start_hour, $_start_minute, $timezone, $do_timezone_conv == 1);
        ...
        if (($end_year !== null) && ($end_month !== null) && ($end_day !== null)) {
            if ($end_monthly_spec_type == 'day_of_month') {
                ...
                $end_month += $dif_month;
                $end_day += $dif_day;
                list($end_minute, $end_hour, $end_month, $end_day, $end_year) = normalise_time_array([$end_minute, $end_hour, $end_month, $end_day, $end_year], $timezone);
            } else {
                // Work out using deltas
                $end_day = $start_day_of_month + $dif_days;
                $end_month = $start_month;
...
function create_selection_list_event_types(?int $it = null, ?int $updated_since = null, bool $adding = false) : object
{
    ...
    foreach ($types as $type) {
        ...
        if (!has_submit_permission('low', get_member(), get_ip_address(), 'cms_calendar', ['calendar', $type['id']])) {
            continue;
        }
...
function date_range(int $from, ?int $to, bool $do_time = true, bool $force_absolute = false, string $timezone = '') : string
{
    ...
    return do_lang('EVENT_TIME_RANGE' . (($timezone == '') ? '' : '_WITH_TIMEZONE'), $date1, $date2, [$_length, $timezone]);
}
            ...
            if (($http_response->download_mime_type == 'text/calendar') || ($http_response->download_mime_type == 'application/octet-stream')) {
                ...
                foreach ($events as $key => $items) {
                    ...
                    foreach ($nodes as $_child) {
                        ...
                        if (array_key_exists("1", $child) && $child[0] !== 'PRODID' && $child[0] !== 'VERSION' && $child[0] !== 'END') {
                            $calendar_nodes[$key][$child[0]] = str_replace(['\n', '\,'], ["\n", ','], trim($child[1]));
                        }
                    }
                    if ($key != 0) {
                ...
                foreach ($rss->gleamed_items as $item) {
                    ...
                    if ((array_key_exists('title', $item)) && (array_key_exists('clean_add_date', $item)) && ($full_url != '')) {
                        $event = ['e_recurrence' => 'none', 'e_content' => array_key_exists('news', $item) ? $item['news'] : '', 'e_title' => $item['title'], 'e_id' => $full_url, 'e_priority' => 'na', 't_logo' => 'icons/calendar/rss', 'e_recurrences' => 1, 'e_seg_recurrences' => '', 'e_timezone' => get_users_timezone(), 'validated' => 1];
                        if ($event_type !== null) {
                            $event['t_logo'] = $_event_types[$event_type]['t_logo'];
                        }
...
function detect_conflicts(int $member_id, ?int $skip_id, ?int $start_year, ?int $start_month, ?int $start_day, string $start_monthly_spec_type, ?int $start_hour, ?int $start_minute, ?int $end_year, ?int $end_month, ?int $end_day, string $end_monthly_spec_type, ?int $end_hour, ?int $end_minute, string $recurrence, ?int $recurrences, int $type, ?int $member_calendar, int $scope_type) : ?object
{
    ...
    foreach ($conflicts as $conflict) {
        ...
        if ($event['e_member_calendar'] === null) {
            switch ($scope_type) {
                ...
                case DETECT_CONFLICT_SCOPE_SAME_MEMBER_OR_SAME_TYPE_IF_GLOBAL:
                    if ($member_calendar === null) { // if neither global
        ...
        $url = build_url(['page' => 'calendar', 'type' => 'view', 'id' => $id], get_module_zone('calendar'));
        $conflict = (!$protected) ? make_string_tempcode(get_translated_text($event['e_title'])) : do_lang_tempcode('PRIVATE_HIDDEN');
        $out->attach(do_template('CALENDAR_EVENT_CONFLICT', ['_GUID' => '2e209eae2dfe2ee74df61c0f4ffe1651', 'URL' => $url, 'ID' => strval($id), 'TITLE' => $conflict]));
    }
...
function normalise_time_array(array $arr, string $zone) : array
{
    ...
    return [$minute, $hour, $month, $day, $year];
}
...
function cal_get_start_utctime_for_event(string $timezone, int $year, int $month, int $day, string $monthly_spec_type, ?int $hour, ?int $minute, bool $show_in_users_timezone) : int
{
    ...
    if ((!$show_in_users_timezone) || (get_option('enable_timezones') === '0')) { // Move into timezone, as if that is UTC, as it won't get converted later
...
function cal_get_end_utctime_for_event(string $timezone, int $year, int $month, int $day, string $monthly_spec_type, ?int $hour, ?int $minute, bool $show_in_users_timezone) : int
{
    ...
    if ((!$show_in_users_timezone) || (get_option('enable_timezones') === '0')) { // Move into timezone, as if that is UTC, as it won't get converted later (in cal_utctime_to_usertime)
        $timestamp = tz_time($timestamp, $timezone);
    }
    ...
    foreach ($events as $event) {
        ...
        foreach ($our_times as $our) {
            foreach ($their_times as $their) {
                ...
                if (($our[3] === null) && ($their[3] === null)) { // Has to be exactly the same
                    if ($our[2] == $their[2]) {
                        $conflict = true;
                    }
                } elseif (($our[3] === null) && ($their[3] !== null)) { // Ours has to occur within their period
                    if (($our[2] >= $their[2]) && ($our[2] < $their[3])) {
                        $conflict = true;
                    }
                } elseif (($our[3] !== null) && ($their[3] === null)) { // Theirs has to occur within our period
                    if (($their[2] >= $our[2]) && ($their[2] < $our[3])) {
                        $conflict = true;
                    }
                } elseif (($our[3] !== null) && ($their[3] !== null)) { // The two periods need to overlap
                    if (($our[2] >= $their[2]) && ($our[2] < $their[3])) {
                        $conflict = true;
                    }
                    if (($their[2] >= $our[2]) && ($their[2] < $our[3])) {
                ...
                if ($conflict) {
                    $conflicts[] = [$event['e_id'], $event, $their[2], $their[3]];
                    break 2;
                }
            }
        }
...
function adjust_event_dates_for_a_recurrence(string $day, array $event, string $timezone) : array
{
    ...
    if (count($explode) == 3) {
        ...
        $has_end_date = ($event['e_end_year'] !== null) && ($event['e_end_month'] !== null) && ($event['e_end_day'] !== null);
