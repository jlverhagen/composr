...
function find_member_subscriptions(int $member_id, bool $usergroup_subscriptions_only = false) : array
{
    ...
    if ((get_forum_type() == 'cns') && (addon_installed('ecommerce'))) {
        ...
        $_subscriptions_non_recurring = $GLOBALS['FORUM_DB']->query_select('f_group_member_timeouts', ['*'], ['member_id' => $member_id]);
        foreach ($_subscriptions_non_recurring as $sub) {
            $found_transaction = false;
            $subs_trans = $GLOBALS['SITE_DB']->query_select('ecom_transactions', ['*'], ['t_purchase_id' => $member_id, 't_status' => 'Completed'], 'ORDER BY t_time DESC');
            foreach ($subs_trans as $sub_trans) {
                $matches = [];
                if (preg_match('#^USERGROUP(\d+)$#', $sub_trans['t_type_code'], $matches) != 0) {
                    $sub_trans_2 = $GLOBALS['FORUM_DB']->query_select('f_usergroup_subs', ['*'], ['id' => intval($matches[1])], '', 1);
        ...
        foreach ($_subscriptions as $sub) {
            ...
            if (substr($sub['s_type_code'], 0, 9) == 'USERGROUP') {
                ...
                if ($usergroup_subscription_rows === null) {
                    $usergroup_subscription_rows = list_to_map('id', $GLOBALS['FORUM_DB']->query_select('f_usergroup_subs', ['*']));
                }
                ...
                $usergroup_rows = $GLOBALS['FORUM_DB']->query_select('f_groups', ['*'], ['id' => $usergroup_id], '', 1);
