...
function cns_edit_topic(?int $topic_id, ?string $description = null, ?string $emoticon = null, ?int $validated = null, ?int $open = null, ?int $pinned = null, ?int $cascading = null, string $reason = '', ?string $title = null, ?string $description_link = null, bool $check_perms = true, ?int $views = null, bool $null_is_literal = false)
{
    ...
    if (($validated !== null) && ($validated == 1)) {
        $GLOBALS['FORUM_DB']->query_update('f_posts', ['p_validated' => 1], ['id' => $info[0]['t_cache_first_post_id']], '', 1); // Auto-validate first post, if topic validated
    }
    ...
    $GLOBALS['FORUM_DB']->query_update('f_topics', $update, ['id' => $topic_id], '', 1);
...
function cns_delete_topic(int $topic_id, string $reason = '', ?int $post_target_topic_id = null, bool $check_perms = true, bool $reverse_point_transaction = false) : ?int
{
    ...
    if ($forum_id !== null) {
        ...
        if (!array_key_exists(0, $post_count_info)) {
            $post_count_info = [['id' => $forum_id, 'f_post_count_increment' => 1]];
        }
    ...
    if ($post_target_topic_id !== null) { // If we were asked to move the posts into another topic
        $GLOBALS['FORUM_DB']->query_update('f_posts', ['p_cache_forum_id' => $to, 'p_topic_id' => $post_target_topic_id], ['p_topic_id' => $topic_id]);
    ...
    $GLOBALS['SITE_DB']->query_update('url_id_monikers', ['m_deprecated' => 1], ['m_resource_page' => 'topicview', 'm_resource_type' => 'browse', 'm_resource_id' => strval($topic_id)]);
...
function cns_move_topics(int $from, int $to, ?array $topics = null, bool $check_perms = true) // NB: From is good to add a additional security/integrity. We'll never move from more than one forum. Extra constraints that cause no harm are good in a situation that doesn't govern general efficiency.
{
    ...
    if ($topics === null) { // All of them
        ...
        $GLOBALS['FORUM_DB']->query_update('f_topics', ['t_forum_id' => $to], ['t_forum_id' => $from]);

        // Update forum IDs' for posts
        $GLOBALS['FORUM_DB']->query_update('f_posts', ['p_cache_forum_id' => $to], ['p_cache_forum_id' => $from]);
        ...
        // Update forum IDs' for posts
        $GLOBALS['FORUM_DB']->query_update('f_posts', ['p_cache_forum_id' => $to], ['p_topic_id' => $topics[0]]);
...
function cns_invite_to_pt(int $member_id, int $topic_id)
{
    ...
    $test = $GLOBALS['FORUM_DB']->query_select_value_if_there('f_special_pt_access', 's_member_id', [
    ...
    $GLOBALS['FORUM_DB']->query_insert('f_special_pt_access', [
        's_member_id' => $member_id,
        's_topic_id' => $topic_id,
    ]);
    ...
    $_topic_url = build_url(['page' => 'topicview', 'type' => 'view', 'id' => $topic_id], get_module_zone('topicview'), [], false, false, true);
    $topic_url = $_topic_url->evaluate();
    $topic_title = $topic_info[0]['t_cache_first_title'];

    require_code('cns_posts_action');
    $post = do_lang('INVITED_TO_PT', $username, $current_displayname, $current_username, $displayname);
    cns_make_post($topic_id, '', $post, 0, false, 1, 1, do_lang('SYSTEM'), null, null, db_get_first_id($GLOBALS['FORUM_DB']->driver), null, null, null, false, true, null, true, '', null, false, false, true);
...
function send_pt_notification(int $post_id, string $subject, int $topic_id, int $to_id, ?int $from_id = null, ?string $post_comcode = null, bool $mark_unread = false)
{
    ...
    if ($post_comcode === null) {
        $post_comcode = get_translated_text($GLOBALS['FORUM_DB']->query_select_value('f_posts', 'p_post', ['id' => $post_id]), $GLOBALS['FORUM_DB']);
    }

    $emphasised = ($GLOBALS['FORUM_DB']->query_select_value('f_posts', 'p_is_emphasised', ['id' => $post_id]) == 1);
