...
function _helper_apply_emoticons(object $this_ref, ?int $member_id = null) : array
{
    ...
    $this_ref->EMOTICON_CACHE = [];
    $EMOTICON_LEVELS = [];

    $query = 'SELECT e_code,e_theme_img_code,e_relevance_level FROM ' . $this_ref->db->get_table_prefix() . 'f_emoticons WHERE e_relevance_level<4' . $extra . ' ORDER BY ' . db_function('LENGTH', ['e_code']) . ' DESC';
...
function _helper_get_forum_topic_posts(object $this_ref, ?int $topic_id, ?int &$count, ?int $max, int $start, bool $mark_read = true, bool $reverse = false, bool $light_if_threaded = false, ?array $post_ids = null, bool $load_spacer_posts_too = false, string $sort = 'date')
{
    ...
    if (!$load_spacer_posts_too) {
        $where .= not_like_spacer_posts($this_ref->db->translate_field_ref('p_post'));
    }
    $where .= $extra_where;
    ...
    if (($is_threaded) || ($sort == 'compound_rating') || ($sort == 'average_rating')) {
        $select .= ',' . db_function('COALESCE', ['(SELECT AVG(rating) FROM ' . $this_ref->db->get_table_prefix() . 'rating WHERE ' . db_string_equal_to('rating_for_type', 'post') . ' AND rating_for_id=' . db_cast('p.id', 'CHAR') . ')', '5']) . ' AS average_rating';
...
function _helper_get_emoticon_chooser(object $this_ref, string $field_name) : object
{
    ...
    $extra_where = $use_special ? [] : ['e_is_special' => 0];
    $emoticons = $this_ref->db->query_select('f_emoticons', ['*'], ['e_relevance_level' => 0] + $extra_where, 'ORDER BY e_code');
    $em = new Tempcode();
    foreach ($emoticons as $emo) {
        ...
        if ($emo['e_theme_img_code'] !== '') {
            $em->attach(do_template('EMOTICON_CLICK_CODE', ['_GUID' => '1a75f914e09f2325ad96ad679bcffe88', 'FIELD_NAME' => $field_name, 'CODE' => $code, 'IMAGE' => apply_emoticons($code)]));
        }
    }
