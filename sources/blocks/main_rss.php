...
class Block_main_rss
{
    ...
    {
        $info = [];
        $info['cache_on'] = ['Block_main_rss::block_main_rss__cache_on'];
    ...
    public function run(array $map) : object
    {
        ...
        foreach (array_reverse($urls) as $url) { // Reversed so that $rss stays as the first feed, and hence the title etc comes from the first, not the last
            ...
            $rss = object_factory('Source_RSS', false, [$url, false, 6.0 / floatval(count($urls))], true);
            if ($rss->error !== null) {
                $error = $rss->error;
     ...
     */
    public static function block_main_rss__cache_on(array $map) : array
    {
