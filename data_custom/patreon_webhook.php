...
if (!addon_installed('patreon')) {
    warn_exit(do_lang_tempcode('MISSING_ADDON', escape_html('patreon')));
}

header('X-Robots-Tag: noindex');

if (empty($_SERVER['HTTP_X_PATREON_SIGNATURE'])) {
    fatal_exit('Patreon signature missing');
}
$signature_given = $_SERVER['HTTP_X_PATREON_SIGNATURE'];

$body = file_get_contents('php://input');

$found_secret = false;
$alternate_config = null;
foreach (explode(',', get_option('patreon_webhook_secret')) as $secret) {
    $matches = [];
    if (preg_match('#^\s*([^=]*?)\s*=\s*([^\s=]*)\s*$#', $secret, $matches) != 0) {
        $_alternate_config = trim($matches[1]);
        $secret = trim($matches[2]);
    } else {
        $_alternate_config = 'admin';
        $secret = trim($secret);
    }

    if ($secret != '') {
        $found_secret = true;
        if (hash_hmac('md5', $body, $secret) == $signature_given) {
            $alternate_config = $_alternate_config;
        }
    }
}
if (!$found_secret) {
    fatal_exit('No Patreon secret configured');
}
if ($alternate_config === null) {
    fatal_exit('Patreon signature mismatch (secret misconfigured or hack-attempt)');
}
...
patreon_sync_individual_member($user_id, $alternate_config);
