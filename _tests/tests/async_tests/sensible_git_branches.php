...
class sensible_git_branches_test_set extends cms_test_case
{
    public function testSensibleBranches()
    {
        ...
        foreach ($branches as $branch) {
            $this->assertTrue((preg_match('#_v[\d\.]+$#', $branch['git_branch']) === 1) || (preg_match('#^v[\d\.]+$#', $branch['git_branch']) === 1), $branch['git_branch'] . ' does not seem to have the expected version status');
        }
    }
}
